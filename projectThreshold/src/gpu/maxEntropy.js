...
const maxEntropy = (raster,graphContext, copy_mode = true) => {
  ...
  const getFragmentSource = (samplerType,outVec) => {
    ...
    void main() {
      ...
      pixelColor[0] > threshold ? outColor = vec4(1.0,1.0,1.0,1) : outColor = vec4(0.0,0.0,0.0,1.0);;
    }`;
  }
...
// Run maxEntropy
maxEntropy(img.getRaster(),gpuEnv);
